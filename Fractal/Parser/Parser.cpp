...
uint8_t tokenBindingPower(const Token& token) {
    switch (token.type) {
        case DOT:
        case ARROW:           return 110;
        case AT:
        case AMPERSAND:       return 100;
        case STAR:
        case SLASH:           return 80;
        case PLUS:
        case MINUS:           return 70;
        case GREATER:
        case LESS:
        case GREATER_EQUAL:
        case LESS_EQUAL:      return 60;
        case EQUAL_EQUAL:
        case BANG_EQUAL:      return 50;
        case AND:             return 40;
        case OR:              return 30;
        case EQUAL:           return 20;
        default:              return 0;
    }
}

const StatementList& Parser::statements() const { return m_programFile.statements; }
const DefinitionList& Parser::definitions() const { return m_programFile.definitions; }
ProgramFile& Parser::program() { return m_programFile; }

void Parser::advance() {
    m_currentIndex++;
    if (m_currentIndex < (int32_t)m_tokenList.size())
...
const Token& Parser::currentToken() const { return *m_currentToken; }
...
ExpressionPtr Parser::parseExpression(BindingPower bindingPower) {
    ...
    while (tokenBindingPower(currentToken()) > bindingPower) {
        ...
        advance();
        // led consumes ownership of `left` and returns a new node wrapping it.
...
ExpressionPtr Parser::expressionLiteral(const Token& token) {
    switch (token.type) {
        ...
        case CHARACTER_LITERAL:
            return std::make_unique<CharacterLiteral>(token.value, token.position);
...
ExpressionPtr Parser::expressionAddressOf() {
    ExpressionPtr expr = parseExpression(100);
    return std::make_unique<AddressOfExpression>(std::move(expr));
}
...
ExpressionPtr Parser::expressionCall(const Token& token) {
    ...
    consume(RIGHT_PARENTHESIS, "Expected ')'");
    return std::make_unique<Call>(token, std::move(argList));
}
...
ExpressionPtr Parser::expressionArray() {
    ...
    while (!match(RIGHT_BRACKET) && !match(SPECIAL_EOF)) {
        ...
        expressions.push_back(ArrayElement{ std::move(expr), second });
        if (match(COMMA)) advance();
        else if (!match(RIGHT_BRACKET)) break;
    }

    consume(RIGHT_BRACKET, "Expected ']'");
    return std::make_unique<ArrayList>(std::move(expressions));
}
...
StatementPtr Parser::parseStatement() {
    switch (currentToken().type) {
        case SEMICOLON:   return statementNull();
        case LEFT_BRACE:  return statementCompound();
        case RETURN:      return statementReturn();
        case IF:          return statementIf();
        case LOOP:        return statementLoop();
        case WHILE:       return statementWhile();
        case BREAK:       return statementBreak();
        case CONTINUE:    return statementContinue();
        case LET:
        case CONST:
...
StatementPtr Parser::statementExpression() {
    ...
    CONSUME_SEMICOLON();
    return std::make_unique<ExpressionStatement>(std::move(expression), pos);
}
...
void Parser::handleDefinitions() {
    ...
    if (!(currentToken().type == LESS && peek().type == BANG
          && peek(2).value == "define" && peek(3).type == GREATER)) {
        m_errorHandler->reportError(
            { "Expected end of definition set '<!define>'", currentToken().position });
    } else {
...
DefinitionPtr Parser::parseDefinition() {
    switch (currentToken().type) {
        case AT:       return parseDecoratedDefinition();
        case FUNCTION: return definitionFunction();
        case LET:
        case CONST:    return definitionVariable(true);
        case CLASS:    return definitionClass();
        default:       return nullptr;
    }
}
...
DefinitionPtr Parser::parseDecoratedDefinition() {
    ...
    advance();
    return std::make_unique<DecoratedDefinition>(decorator, parseDefinition());
}
...
DefinitionPtr Parser::definitionFunction() {
    ...
    return std::make_unique<FunctionDefinition>(
        *nameToken, std::move(parameterList), returnType, parseStatement());
}
...
DefinitionPtr Parser::definitionVariable(bool isGlobal) {
    ...
    if (match(EQUAL)) {
        ...
        m_errorHandler->reportError(
            { "A type must be specified if no initializer is given", currentToken().position });
    }

    CONSUME_SEMICOLON();

    return std::make_unique<VariableDefinition>(
        *nameToken, variableType, std::move(initializer), isConst, isGlobal);
}
...
DefinitionPtr Parser::definitionClass() {
    ...
    return std::make_unique<ClassDefinition>(nameToken->value, std::move(classMembers));
}
